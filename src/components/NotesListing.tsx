...
const NotesListing = ({
    ...
    const deleteItem = (id: any) => {
        if (!isOnline) {
            Alert.alert('You are offline. Please try again later.');
            return;
        }
    ...
    const Listing = ({ item }: any) => (
        <View
            style={[
                styles.noteCard,
                { backgroundColor: theme.mode === 'dark' ? '#111111' : '#F4F4F4' },
            ]}
        >
            ...
            <View style={styles.options}>
                <Text style={styles.Edit} onPress={() => openModal(item)}>
                    Edit
                </Text>
    ...
    return (
        <View style={styles.mainView}>
            <FlatList
                ...
                onRefresh={() => {
                    if (isOnline) dispatch(fetchNotes());
                }}
                renderItem={Listing}
                ListEmptyComponent={
                    !loading ? (
                        <>
                            {!isOnline && (
                                <Text style={styles.OfflineError}>
                                    You are offline. Connect to the internet to view notes.
                                </Text>
                            )}
                            {isOnline && CustomeMessage ? (
                                <Text
                                    style={[
                                        styles.emptyText,
                                        { color: theme.text },
